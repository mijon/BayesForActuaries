...
- Choose a candidate jump location
...
- If candidate is larger than current, go to candidate

--

- If candidate is smaller than current, go to candidate with some probability
...
s <- svgstring(standalone = FALSE)
ggplot(df, aes(index, visited)) + geom_line() + facet_wrap(~slice, scales='free_x') + theme_bw()
htmltools::HTML(s())
```
...
s <- svgstring(standalone = FALSE)
ggplot(islands_frequencies) + geom_col(aes(island, value)) + facet_wrap(~stage, scales='free_y') + theme_bw()
htmltools::HTML(s())
```
...
# error
base_sd <- 10
outlier_sd_multiple <- 3

error <- rnorm(npoints, mean=0, sd=base_sd)
error[1:round(outlier_rate * npoints)] <- abs(rnorm(round(outlier_rate * npoints), mean=0, sd=outlier_sd_multiple*base_sd))


# Fake data generation
x <- runif(npoints, x_low, x_high)
...
linear_plot <-ggplot(data) + geom_point(aes(x,y)) + theme_bw()
...
```{r echo=FALSE}
s <- svgstring(standalone = FALSE)
linear_plot + geom_smooth(aes(x,y), method='lm', se=FALSE) + geom_abline(slope=beta_1, intercept=beta_0)
htmltools::HTML(s())
```
---
...
Reproduce Gesmann's work in a Bayesian way
